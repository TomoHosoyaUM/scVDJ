## usage:   Rscript 55a_remove_checked.r

system(paste0("echo @@@ 55a_remove_error.r >> ./analysis.log.txt"))
system(paste0("echo @@@ 55a_remove_error.r"))

#################################################################################################
## output directory #############################################################################
path1="./table55_sampleID_checked/"
...
## final tbl write.csv ##########################################################################
write.csv(tbl,file=paste0(path1,"55_Cluster_selected.csv"),row.names=F)
write.csv(tbl,file=paste0(path1,"Cluster_selected.csv"),row.names=F)

N=nrow(tbl)
print(N)
system(paste0("echo ",N," clusters selected in table55 >> ./analysis.log.txt"))
